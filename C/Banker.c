...
int main() {
    ...
    //設定各Process的最大需求數量
    int max[PROC_MAX_COUNT][RES_COUNT];
    ...
    //設定各Process已佔用的資源數量
    int alloc[PROC_MAX_COUNT][RES_COUNT];
    for (int j = 0; j < process_count; j++) {
        for (int k = 0; k < RES_COUNT; k++) {
    ...
    //算出需求矩陣
    int need[PROC_MAX_COUNT][RES_COUNT];
    for (int j = 0; j < process_count; j++) {
        for (int k = 0; k < RES_COUNT; k++) {
